...
class FolderImageRenamer:
    ...
    def setup_ui(self):
        ...
        self.rename_btn = tk.Button(button_frame, text="Copy and Rename Folder", command=self.copy_and_rename_folder)
    ...
    def on_subfolder_select(self, event):
        ...
        if selection:
            ...
            # Optionally, load images from selected subfolder
            # For simplicity, using main folder images
            self.load_images()
            self.display_image(0)
    ...
    def copy_and_rename_folder(self):
        ...
        try:
            # Copy entire folder to destination with new name
...
if __name__ == "__main__":
    import PIL  # To ensure Pillow is imported
    root = tk.Tk()
